...
import { webSearch, webSearchSchema, type WebSearchArgs } from "./tools/web_search.js";
import { fetchUrl, fetchUrlSchema, type FetchUrlArgs } from "./tools/fetch_url.js";
import { libDocs, libDocsSchema, type LibDocsArgs } from "./tools/lib_docs.js";
import { searchDocs, searchDocsSchema, type SearchDocsArgs } from "./tools/search_docs.js";
import { listDocs, listDocsSchema, type ListDocsArgs } from "./tools/list_docs.js";
...
const server = new Server(
  { name: "web-docs-mcp", version: "1.0.0" },
  { capabilities: { tools: {} } },
);
...
const TOOLS = [
  {
    name: "web_search",
    description:
      "Free web search via DuckDuckGo HTML (no API key, no per-query cost). " +
      "Returns a list of {title, url, snippet}. Pass any URL to `fetch_url` for full content.",
    schema: webSearchSchema,
    handler: (args: WebSearchArgs) => webSearch(args),
  },
  {
    name: "fetch_url",
    description:
      "Fetch a single URL and return clean markdown. Handles HTML (via turndown+GFM), " +
    ...
    handler: (args: FetchUrlArgs) => fetchUrl(args),
  },
  {
    name: "lib_docs",
    description:
      "Fetch README/docs for a library by name. Tries npm → PyPI → crates.io → Go → GitHub " +
      "automatically. Accepts `react`, `numpy`, `serde`, `owner/repo`, etc. " +
      "Optionally saves to docs/libraries/<name>.md.",
    schema: libDocsSchema,
    handler: (args: LibDocsArgs) => libDocs(args),
  },
  {
    name: "search_docs",
    description:
      "Language & API documentation search. Biases DuckDuckGo results to official docs sites " +
    ...
    handler: (args: SearchDocsArgs) => searchDocs(args),
  },
  {
    name: "list_docs",
    description:
      "Browse and search the local docs/ folder. Three modes: (1) no args = list all saved docs, " +
    ...
    handler: (args: ListDocsArgs) => listDocs(args),
  },
] as const;
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    const text = await tool.handler(args as any);
    return {
