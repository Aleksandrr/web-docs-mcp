...
export type DocsSubdir = (typeof DOCS_SUBDIRS)[keyof typeof DOCS_SUBDIRS];

/** Directory for the TTL cache (raw HTML + fetched markdown). */
export const CACHE_DIR: string =
  process.env.CACHE_DIR ?? path.resolve(process.cwd(), ".cache", "web-docs");

/** Cache time-to-live in hours. Default: 7 days. */
export const CACHE_TTL_HOURS: number = envInt("CACHE_TTL_HOURS", 24 * 7);

/** HTTP timeout per request, in milliseconds. */
export const HTTP_TIMEOUT_MS: number = envInt("HTTP_TIMEOUT_MS", 20_000);

/** Maximum number of HTTP redirects to follow. */
export const HTTP_MAX_REDIRECTS: number = envInt("HTTP_MAX_REDIRECTS", 5);
...
export const DEFAULT_SEARCH_LIMIT: number = envInt("DEFAULT_SEARCH_LIMIT", 8);

/** Hard cap on search results even if the agent asks for more. */
export const MAX_SEARCH_LIMIT: number = envInt("MAX_SEARCH_LIMIT", 20);

/** Maximum size (in chars) of a single fetched markdown response. */
export const MAX_MD_CHARS: number = envInt("MAX_MD_CHARS", 60_000);
...
export const DEFAULT_SAVE_TO_DOCS: boolean = envBool("DEFAULT_SAVE_TO_DOCS", true);

/** If true, the local cache is bypassed (useful for debugging). */
export const DISABLE_CACHE: boolean = envBool("DISABLE_CACHE", false);
