...
export interface SaveDocOptions {
  /** Display name used to derive the slug (e.g. library name, page title). */
  name: string;
  /** Subdirectory under DOCS_DIR (e.g. "libraries", "guides"). Default: "" */
  subdir?: string;
  /** Source URL the content was fetched from. */
  sourceUrl?: string;
  /** Optional content-type for the frontmatter. */
  contentType?: string;
  /** Tool that produced this doc, for traceability. */
  tool?: string;
...
export interface SaveDocResult {
  ...
  created: boolean;
  /** Final slug (without `.md` extension). */
  slug: string;
}
...
function frontmatter(opts: SaveDocOptions): string {
  const lines: string[] = ["---"];
  lines.push(`title: ${yamlScalar(opts.name)}`);
  if (opts.sourceUrl) lines.push(`source: ${yamlScalar(opts.sourceUrl)}`);
  if (opts.contentType) lines.push(`content_type: ${yamlScalar(opts.contentType)}`);
  if (opts.tool) lines.push(`fetched_by: ${yamlScalar(opts.tool)}`);
