...
export interface Doc {
  ...
  path: string;
  /** Path relative to DOCS_DIR, e.g. "libraries/express.md". */
  relativePath: string;
  /** Filename without extension, e.g. "express". */
  slug: string;
  /** Subdir under DOCS_DIR (e.g. "libraries"). Empty if at root. */
  subdir: string;
  /** Parsed YAML frontmatter (empty object if none). */
  frontmatter: Frontmatter;
  /** Markdown body (without frontmatter). */
  body: string;
  /** First heading or filename-derived title. */
  title: string;
}
...
export function parseDoc(fullPath: string): Doc | null {
  let raw: string;
  try {
...
export function findBySlug(slug: string, subdir?: string): Doc | undefined {
  const lower = slug.toLowerCase();
  return indexAll().find(
    (d) =>
...
export function findBySourceUrl(url: string): Doc | undefined {
  const u = normalizeUrl(url);
  return indexAll().find(
    (d) => d.frontmatter.source && normalizeUrl(d.frontmatter.source) === u,
  );
}
 ...
 */
export function searchByKeywords(query: string, limit: number = 10): Array<Doc & { score: number; matchedIn: string[] }> {
  const tokens = tokenize(query);
  if (tokens.length === 0) return [];

  const results: Array<Doc & { score: number; matchedIn: string[] }> = [];
  for (const doc of indexAll()) {
    ...
    for (const t of tokens) {
      ...
      if (slugLower.includes(t)) {
        score += 4;
        if (!matchedIn.includes("slug")) matchedIn.push("slug");
      }
      ...
      if (count > 0) {
        ...
        if (!matchedIn.includes("body")) matchedIn.push(`body (${count}×)`);
      }
    }
    if (score > 0) {
...
export function listAll(subdir?: string): Doc[] {
  const all = indexAll();
  if (!subdir) return all;
