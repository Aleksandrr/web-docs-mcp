...
export const webSearchSchema = {
  ...
  limit: z
    ...
    .optional()
    .describe(
      `Maximum number of results to return. Default ${DEFAULT_SEARCH_LIMIT}, hard cap ${MAX_SEARCH_LIMIT}.`,
    ),
};
...
export async function webSearch(args: WebSearchArgs): Promise<string> {
  ...
  lines.push(
    `---`,
    `Tip: pass any URL above to the \`fetch_url\` tool to get the full page as markdown.`,
  );
  return lines.join("\n");
}
