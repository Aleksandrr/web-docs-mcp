...
export async function ddgSearch(query, limit = 8) {
    ...
    try {
        ...
        clearTimeout(timer);
        throw new Error(`DuckDuckGo search failed: ${err instanceof Error ? err.message : String(err)}`);
    }
    clearTimeout(timer);
 ...
 */
export function parseDdgHtml(html, _sourceUrl = "") {
    ...
    $(".result, .web-result").each((_, el) => {
        const $el = $(el);
        if ($el.hasClass("result--ad") || $el.hasClass("zci"))
            return;
        ...
        const displayUrl = $el.find(".result__url").first().text().trim() || url;
