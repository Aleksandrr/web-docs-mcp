...
service.addRule("stripNoise", {
    filter: (node) => {
        ...
        if (NOISE_SELECTORS.includes(tag))
            return true;
        const cls = (node.getAttribute?.("class") || "").toLowerCase();
        if (/\b(sidebar|cookie|banner|advert|ads|social|share|related|newsletter|subscribe)\b/.test(cls)) {
            return true;
        }
        return false;
    },
...
export function htmlToMarkdown(html) {
    ...
    let slice = html;
    const mainMatch = /<(?:main|article)[^>]*>([\s\S]*?)<\/(?:main|article)>/i.exec(html);
    if (mainMatch) {
        slice = mainMatch[1];
    }
    else {
        const contentMatch = /<div[^>]*(?:id="content"|class="[^"]*(?:content|documentation|markdown-body|prose|entry-content|post-content)[^"]*")[^>]*>([\s\S]*?)<\/div>/i.exec(html);
        if (contentMatch)
            slice = contentMatch[1];
    }
    let md;
    try {
