...
export function looksLikeAnubis(html) {
    return (/\/\.within\.website\/x\/cmd\/anubis\//i.test(html) &&
 ...
 */
export function isAnubisDeny(html) {
    if (!looksLikeAnubis(html))
        return false;
...
export function parseAnubisChallenge(html, _sourceUrl) {
    ...
    if (bpMatch)
        basePrefix = bpMatch[1].trim().replace(/^["']|["']$/g, "");
    let version = "";
    const vMatch = /<[^>]*id=["']anubis_version["'][^>]*>([^<]*)</.exec(html);
    if (vMatch)
        version = vMatch[1].trim().replace(/^["']|["']$/g, "");
 ...
 */
export function solveAnubis(challenge) {
    ...
    while (true) {
        ...
        if (nonce > 100_000_000) {
            throw new Error(`Anubis PoW exceeded 100M iterations (difficulty=${difficulty})`);
        }
    }
}
/**
 ...
 */
export async function solveAndPassChallenge(challenge, originalUrl) {
    const solution = solveAnubis(challenge);
    const { basePrefix } = challenge;
    const submitUrl = new URL(`${basePrefix}/.within.website/x/cmd/anubis/api/pass-challenge`, originalUrl);
    ...
    try {
        ...
        clearTimeout(timer);
        throw new Error(`Anubis pass-challenge failed: ${err instanceof Error ? err.message : String(err)}`);
    }
    clearTimeout(timer);
