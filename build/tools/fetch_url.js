...
import { DEFAULT_SAVE_TO_DOCS, DOCS_DIR, DOCS_SUBDIRS, MAX_MD_CHARS, } from "../config.js";
export const fetchUrlSchema = {
    ...
    save: z
        .boolean()
        .optional()
        .describe(`If true, also write the markdown to ${DOCS_DIR}/<slug>.md with frontmatter. Default ${DEFAULT_SAVE_TO_DOCS}.`),
    name: z
        .string()
        .optional()
        .describe("Filename (without extension) to use when saving. Defaults to the page <title> or last URL segment."),
    subdir: z
        .string()
        .optional()
        .describe("Optional subdirectory under docs/ (e.g. 'guides', 'libraries', 'api', 'specs', 'snippets'). Default 'guides'."),
    no_anubis: z
        .boolean()
        .optional()
        .describe("If true, disable automatic Anubis PoW bypass. Useful for debugging or when you want the raw challenge page. Default false."),
    refresh: z
        .boolean()
        .optional()
        .describe("If true, skip the local docs/ lookup (by source URL) and force a fresh fetch. Default false."),
};
export async function fetchUrl(args) {
    ...
    const header = `# ${args.name || lastUrlSegment(res.url)}\n` +
        `**Source:** ${res.url}\n` +
...
function formatLocalFetchResult(originalUrl, doc) {
    const lines = [];
