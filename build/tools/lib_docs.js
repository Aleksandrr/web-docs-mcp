...
export const libDocsSchema = {
    name: z
        .string()
        .min(1)
        .describe("Library name. Examples: `react`, `express`, `numpy`, `serde`, `github.com/gin-gonic/gin`, `owner/repo`."),
    ecosystem: z
        .enum(["auto", "npm", "pypi", "crates", "go", "github"])
        .optional()
        .describe("Hint which registry to consult. Default 'auto' — tries npm, then pypi, then crates, then GitHub."),
    version: z
        .string()
        .optional()
        .describe("Specific version to fetch (npm/PyPI only). Defaults to latest."),
    save: z
        .boolean()
        .optional()
        .describe("If true, also write markdown to docs/libraries/<name>.md. Default: see DEFAULT_SAVE_TO_DOCS env."),
    refresh: z
        .boolean()
        .optional()
        .describe("If true, skip the local docs/ lookup and force a fresh fetch from upstream. Useful when you suspect the cached doc is stale."),
};
export async function libDocs(args) {
...
async function tryNpm(name, version) {
    ...
    let readme = (ver && data.versions?.[ver]?.readme) ||
        data.readme ||
        undefined;
    if (!readme && ver) {
...
async function tryReadmeFromCdn(name, version) {
    ...
    const listing = await fetchJson(`https://data.jsdelivr.com/v1/packages/npm/${encodeURIComponent(name)}@${encodeURIComponent(version)}`).catch(() => undefined);
    if (listing?.files) {
        const readmeFile = listing.files.find((f) => f.type === "file" && /^readme\.(md|markdown|rst|txt)$/i.test(f.name));
        if (readmeFile) {
...
async function tryPypi(name, version) {
    const url = version
...
async function tryCrates(name) {
    ...
    if (!readme)
        readme = c.description || "(no README available)";
    return {
        name: c.name,
        version: ver,
...
async function tryGithub(owner, repo) {
    ...
    if (!readme) {
        ...
        const repoInfo = await fetchJson(`https://api.github.com/repos/${owner}/${repo}`, { headers }).catch(() => undefined);
...
async function tryGithubReadme(owner, repo, branch) {
...
async function tryGithubReadmeFromRepo(repoUrl) {
    if (!repoUrl)
        return undefined;
    ...
    const headers = { "User-Agent": "web-docs-mcp" };
    if (GITHUB_TOKEN)
        headers.Authorization = `Bearer ${GITHUB_TOKEN}`;
    const info = await fetchJson(`https://api.github.com/repos/${owner}/${repo}`, { headers }).catch(() => undefined);
