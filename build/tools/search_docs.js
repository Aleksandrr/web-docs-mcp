...
export const searchDocsSchema = {
    ...
    target: z
        .enum(TARGETS)
        .optional()
        .describe("Which documentation site to bias toward. Default 'auto' (tries MDN first if query looks JS/Web, otherwise general docs)."),
    fetch_top: z
        .boolean()
        .optional()
        .describe("If true, fetch the top hit and return its full markdown instead of just the result list."),
    save: z
        .boolean()
        .optional()
    ...
    local_only: z
        .boolean()
        .optional()
        .describe("If true, ONLY search local docs/ folder — never hit the network. Returns 'not found' if no local matches. Default false."),
    skip_local: z
        .boolean()
        .optional()
        .describe("If true, skip the local docs/ lookup and go straight to web search. Default false."),
};
...
async function webSearchPhase(args, target) {
...
function formatLocalSearchResults(query, hits, returnTopBody) {
    const lines = [];
    ...
    hits.forEach((h, i) => {
        ...
        if (h.frontmatter.source)
            lines.push(`   Source: ${h.frontmatter.source}`);
...
function pickSites(target, query) {
    if (target !== "auto")
        return SITE_MAP[target].split(" OR ");
    // Auto: MDN for web-ish queries, otherwise the big general docs sites.
    const webHit = /\b(array|object|fetch|promise|css|html|dom|javascript|js|window|document|canvas|webgl|service worker)\b/i.test(query);
    if (webHit)
