 ...
 */
import { z } from "zod";
import { indexAll, searchByKeywords, listAll, parseDoc, VALID_SUBDIRS, } from "../lib/local-search.js";
...
export const listDocsSchema = {
    query: z
        .string()
        .optional()
        .describe("If provided, perform a keyword search across all docs/. " +
        "If omitted, list all docs (or those in `subdir`)."),
    subdir: z
        .enum(VALID_SUBDIRS)
        .optional()
        .describe("Filter by subdirectory: 'libraries', 'api', 'guides', 'specs', or 'snippets'. " +
        "Ignored when `query` is set (search always scans all subdirs)."),
    path: z
        .string()
        .optional()
        .describe("If provided, return the full body of this specific doc. " +
...
export async function listDocs(args) {
    ...
    if (args.query) {
        ...
        hits.forEach((h, i) => {
            ...
            if (h.frontmatter.version)
                lines.push(`   Version: ${h.frontmatter.version}`);
    ...
    for (const d of shown) {
        if (!grouped.has(d.subdir))
            grouped.set(d.subdir, []);
    ...
    for (const [sub, items] of [...grouped.entries()].sort()) {
        ...
        for (const d of items) {
            const meta = [];
            if (d.frontmatter.version)
                meta.push(`v${d.frontmatter.version}`);
...
function showDocByPath(userPath) {
...
function formatDocBody(doc) {
    ...
    if (doc.frontmatter.fetched_by)
        lines.push(`**Fetched by:** ${doc.frontmatter.fetched_by} tool`);
    lines.push(``);
    lines.push(`---`);
    lines.push(``);
    lines.push(doc.body);
